...
def item(title, url, source, text=""):
    # Synthetic publishers, not collector buckets: Phase 11 counts URL hosts.
    url = f"https://{source}.example/{url}"
...
def run_all():
    """Execute every check. Returns (passed, failed) name lists."""
    PASS.clear()
    FAIL.clear()
    ...
    check("all answer the seven Phase-11 user questions",
          all(set(o.questions) == {"build", "learn", "automate", "market",
                                   "research", "revenue", "improve_nexara"} for o in opps))
    check("every question answer cites its basis",
          all(isinstance(o.questions, dict)
              and all(isinstance(v, str) and v.strip() for v in o.questions.values())
              for o in opps))
...
def test_phase10_world_intelligence():
    """pytest entry point — fails listing any failed check."""
    _passed, failed = run_all()
